...
/**
 * @author lyf
 * @date 2018-10-10 15:22:12
 * @descripe 计算二十四节气以及农历节日日期
 */
public class SolarTermsUtil {
     ...
     * 用于保存传统节假日
     */
    final static String[] lunarHoliday = new String[] { "0101 春节", "0115 元宵", "0505 端午", "0707 情人", "0715 中元", "0815 中秋", "0909 重阳", "1208 腊八", "1224 小年" };

    /**
     * 用于保存国际节假日
     */
    final static String[] solarHoliday = new String[] { "0101 元旦", "0214 情人", "0308 妇女", "0312 植树", "0315 消费者权益日", "0401 愚人", "0501 劳动", "0504 青年",
     ...
     * @return 24节气的值
     */
    public List<String> getSolartermsName() {
        ...
        if (gregorianDate == sectionalTerm) {
            strList.add(sectionalTermNames[gregorianMonth - 1]);
        } else if (gregorianDate == principleTerm) {
            strList.add(principleTermNames[gregorianMonth - 1]);
        }
        if ("12".equals(cm.trim())) {
            if (lastDate == 29 && cd.trim().equals("29") || lastDate == 30 && cd.trim().equals("30")) {
                strList.add("除夕");
            }
        }
        // 传统节假日
        String lh = loop(lunarHoliday,cm,cd);
        if (!lh.equals("")) {
            strList.add(lh);
        }

        // 国际节假日
        String sh = loop(solarHoliday,gm,gd);
        if (!sh.equals("")) {
            strList.add(sh);
        }
        return strList;
    ...
    public String loop(String[] holidays , String m , String d) {
        String str = "";
        for (int i = 0; i < holidays.length; i++) {
            // 日期arH
            String dateStr = holidays[i].split(" ")[0];
            // 名称
            String name = holidays[i].split(" ")[1];
            String m2 = m.trim();
            String d2 = d.trim();
    ...
    public static void main(String[] args) throws ParseException {
        ...
        today.setTime(df.parse("2010-02-14"));
        SolarTermsUtil solarTermsUtil = new SolarTermsUtil(today);
        List<String> solarTerms = solarTermsUtil.getSolartermsName();
        solarTerms.stream().forEach(System.out::println);
